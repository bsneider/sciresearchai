#!/usr/bin/env python3
"""
Research CLI Wrapper for Gustav Research Executor System

Provides command-line interface for research-focused gustav commands to properly
...
class ResearchExecutorCLI:
    def __init__(self, tasks_dir: Optional[str] = None):
        ...
        if not os.path.exists(self.tasks_dir):
            print(f"❌ Gustav research tasks directory not found: {self.tasks_dir}")
            print("   Make sure you're in a Gustav research project with initialized .tasks directory")
            sys.exit(1)
            
        self._load_data()

    def _load_data(self):
        """Load all Gustav Research JSON files"""
    ...
    def _load_json(self, filename: str) -> Dict:
        ...
        try:
            ...
            print("   Run `/gustav:planner` first to initialize research workflow")
            sys.exit(1)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Gustav Research Executor CLI")
